...
class EditSong extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.info.target === 0 || this.state.disableGetData) {
      return
    }
    const { name, description, link, target } = nextProps.info
    this.setState({ name, description, link, target, disableGetData: true })
  }
  ...
  render() {
    return (
      <Modal
        ...
        <div className='text-xl mb-4 font-bold'>노래 수정</div>
        <div
